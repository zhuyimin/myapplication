...
public final class Api {
	...
	public static final String PREF_3G_UIDS_TWO			= "AllowedUids3GTwo";
	public static final String PREF_WIFI_UIDS_TWO		= "AllowedUidsWifiTwo";
	...
	public static void saveRules(Context ctx) {
		...
		final SharedPreferences prefs = ctx.getSharedPreferences(PREFS_NAME, 0);
		//final DroidApp[] apps = getNotSystemApps(ctx);
		final DroidApp[] apps = getNotSystemApps(ctx);
		Log.i("yimin","apps length:" + apps.length);
		...
		for (int i=0; i<apps.length; i++) {
			if (apps[i].selected_wifi) {
				...
				Log.i("yimin","wifi selected:" + apps[i].uid);
				setFirewallUidChainRule(apps[i].uid,WIFI_TYPE,false);
			}else {
				Log.i("yimin","wifi not selected:" + apps[i].uid);
				setFirewallUidChainRule(apps[i].uid,WIFI_TYPE,true);
			}
			if (apps[i].selected_3g) {
				if (newuids_3g.length() != 0) newuids_3g.append('|');
				newuids_3g.append(apps[i].uid);
				Log.i("yimin","gprs selected:" + apps[i].uid);
				setFirewallUidChainRule(apps[i].uid,GPRS_TYPE,false);
			} else {
				Log.i("yimin","gprs not selected:" + apps[i].uid);
				setFirewallUidChainRule(apps[i].uid,GPRS_TYPE,true);
			}
		}
	...
	public static void saveSystemRules(Context ctx) {

		final SharedPreferences prefs = ctx.getSharedPreferences(PREFS_NAME, 0);
		//final DroidApp[] apps = getNotSystemApps(ctx);
		final DroidApp[] apps = getSystemApps(ctx);
		Log.i("yimin","apps length:" + apps.length);
		// Builds a pipe-separated list of names
		//final StringBuilder newuids_wifi = new StringBuilder();
		final StringBuilder newuids_3g = new StringBuilder();
		for (int i=0; i<apps.length; i++) {
			Log.i("yimin","is Selected"+apps[i].selected_3g);
			if (apps[i].selected_3g) {
				if (newuids_3g.length() != 0) newuids_3g.append('|');
				newuids_3g.append(apps[i].uid);
				Log.i("yimin","gprs selected:" + apps[i].uid);
		...
		final Editor edit = prefs.edit();
		//edit.putString(PREF_WIFI_UIDS, newuids_wifi.toString());
		edit.putString(PREF_3G_UIDS_TWO, newuids_3g.toString());
		edit.commit();
	}
	...
	public static void clearFirewallChain() {
	...
	public static DroidApp[] getSystemApps(Context ctx) {
		Log.i("yimin","get system apps");
/*
		...
		//final String savedUids_wifi = prefs.getString(PREF_WIFI_UIDS, "");
		final String savedUids_3g = prefs.getString(PREF_3G_UIDS_TWO, "");
		...
		try {
			...
			for (final ApplicationInfo apinfo : installed) {
				...
				Log.i("yimin","ppppp is selected:" + app.selected_3g);
			...
			for (int i=0; i<special.length; i++) {
				...
				if (app.uid != -1 && !map.containsKey(app.uid)) {
					...
					if (Arrays.binarySearch(selected_wifi, app.uid) >= 10000) {
						app.selected_wifi = true;
					}
					if (Arrays.binarySearch(selected_3g, app.uid) >= 10000) {
						app.selected_3g = true;
					}
					//map.put(app.uid, app);
				}
	...
	public static DroidApp[] getNotSystemApps(Context ctx) {
/*
		if (applications != null) {
			// return cached instance
			return applications;
	...
	public static boolean assertBinaries(Context ctx, boolean showErrors) {
		...
		try {
			...
			if (changed) {
				Toast.makeText(ctx, "二进制文件已安装!", Toast.LENGTH_LONG).show();
			}
		} catch (Exception e) {
